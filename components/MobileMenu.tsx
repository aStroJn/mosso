...
const MobileMenu: React.FC<MobileMenuProps> = ({ isOpen, onClose, navigateTo, toggleTheme, currentPage }) => {
  ...
  return (
    ...
    >
      ...
      {/* Menu Panel */}
      <div
        className={`relative flex flex-col w-full max-w-sm h-full bg-secondary-background-light dark:bg-secondary-background-dark ml-auto shadow-xl transform transition-transform duration-300 ease-in-out ${
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-border-light dark:border-border-dark">
          <div className="flex items-center cursor-pointer" onClick={() => handleNavigate('home')}>
            <img src="./public/logo.png" alt="MOSSO Logo" className="h-7 w-auto" />
          </div>
